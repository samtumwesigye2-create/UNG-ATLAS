...
SYSTEM_ID='UNG-ATLAS';VERSION='0.9.0';app=FastAPI(title='UNG-ATLAS',version=VERSION)
...
class ServiceIn(BaseModel):
 system_id:str=Field(min_length=2,max_length=80);name:str=Field(min_length=1,max_length=160);base_url:str='';capabilities:list[str]=Field(default_factory=list);kind:str='internal';active:bool=True
...
def probe(s,path='/health'):
 ...
 try:
  with urllib.request.urlopen(u+path,timeout=3) as x:data=json.loads(x.read().decode())
  return {'status':'online' if x.status==200 else 'degraded','latency_ms':round((time.perf_counter()-t)*1000),'data':data}
...
def module(sid:str,idx:int):
 ...
 return f'<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>{C()}</style></head><body><header><a class="back" href="/systems/{sid}">← UNG-{sid}</a><div class="brand">{m}</div><small>{name}</small></header><main><div class="panel"><h1>{m}</h1></div></main></body></html>'
